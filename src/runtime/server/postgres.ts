...
import { postgresSchema } from "~~db/schema.js"
...
const postgresSchema = (await import(config.serverSchema)) as any
export const postgres = useLocal
	? drizzlePgLite({ client: postgresClient, schema: postgresSchema })
	: drizzle({ client: postgresClient, schema: postgresSchema })
...
export const migrate = async (
	...
	if (generateMigration || config.devAutoGenerateMigrations) {
		await run(`pnpm drizzle-kit generate`)
	}

	if (!state.attemptedMigration) {
		const migrationClient = useLocal
			? drizzlePgLite(postgresClient)
